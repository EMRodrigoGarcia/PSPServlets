...
public class Blog extends HttpServlet {
    ...
    public static void printBlog(PrintWriter out) {
        try {
            ...
            String aux = "";
            while(set.next()) {
                out.println(PlantillasHTML.mCarta.replace("$titulo$", set.getString("titulo")).replace("$fecha$", Integer.toString(set.getInt("fecha")).replace("$texto$", set.getString("texto"))));
            }
        } catch (Exception e) {
            //TODO: handle exception
        }
